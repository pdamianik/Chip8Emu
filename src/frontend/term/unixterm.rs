use std::{cmp::min, io::{self, Error, ErrorKind::Interrupted, Read, Write, stdin, stdout}, os::unix::prelude::AsRawFd, sync::{Arc, Mutex, mpsc::{Receiver, Sender}}, thread::{self, sleep}, time::Duration};
use termios::{Termios,cfmakeraw,tcsetattr,TCSANOW};
use crate::{emu::display::DisplayCmd, frontend::FrontendController};
...
impl TermController {
	pub const fn new(changes: Receiver<DisplayCmd>, keyboard_sender: Sender<u8>, beep: Arc<Mutex<bool>>) -> Self {
		Self::console_init();
		Self::render_changes(changes);
		Self::keyboard_init(keyboard_sender);
		Self::bell_init(beep);
		Self {}
	}
	...
	fn render_changes(display_cmds: Receiver<DisplayCmd>) {
		thread::spawn(move || {
			loop {
				...
				match cmd {
					DisplayCmd::Change(_, _, _) => Self::render_change(cmd),
					DisplayCmd::Clear => (),
				}
			}
		});
...
impl FrontendController for TermController {}
